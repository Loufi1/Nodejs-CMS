...
class api {
  ...
  _recalibrateMiddleware() {
    for (let i = this.middlewares.length - 1; i >= 0; i = i - 1) {
      this.middlewares[i].next = this.middlewares[i + 1]
        ? (req, res) =>
            this.middlewares[i + 1].callback(
              req,
              res,
              this.middlewares[i + 1].next
            )
        : undefined;
    }
  }
  ...
  async _redirect(req, res) {
    ...
    const index = this.routes.findIndex(
      (elem) =>
        this._uniformize(elem.route) === this._uniformize(req.url) &&
  ...
  async _redirectParamRoutes(req, res) {
    const index = this.paramRoutes.findIndex((elem) => {
  ...
  _logger(req, res) {
    ...
    let stringStatus = Object.entries(HttpStatusCode).find(
      ([field, value]) => value === statusCode
    )[0];
    ...
    console.log(
      `${date}: ${
        methodColor[method] || '\x1b[0m%s'
      }\x1b[0m - ${colorRoute}\x1b[0m - ${colorCode}\x1b[0m`,
      '[' + method + ']',
      url,
      response
    );
